...
| Q5 | What rate limit does test mode enforce? | Phase 1 measures it: the first 429 under backoff, recorded with the request rate that produced it. | **Open.** No 429 at 1.4 requests per second over 40 sequential calls on 2026-08-31. The probe is `TestLiveRateLimitObservation`, behind `RZP_RATE_LIMIT_PROBE`. A real ramp is phase 2. **Addendum 2026-09-05, after the pivot:** answered by accident rather than by the trigger above. `cmd/seedbook` made 27 unpaced writes and `POST /invoices` answered 429 at about call 17, twice in a row, at a measured 1.9 to 2.1 writes per second. With the 2026-08-31 probe's clean run at 1.3 to 1.4, the write limit on that endpoint sits between those two rates. The four backoff constants were not enough to get through it: `DefaultMaxAttempts` is 4 with 250ms, 500ms and 1s of waiting, which is 1.75s of retry budget in total. The fix is on the calling side, `seed.DefaultPace`, and the run behind this row is a rehearsal that is not committed, so no number here is on a results table. |
