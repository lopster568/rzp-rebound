...
| FR-POLL-2 | Polling runs under a concurrency cap and backs off on 429. | Met in code. The phase 2 live runs set `MaxConcurrent` to 2 rather than the client default of 4. No 429 was observed on the polling path. The 429s Q5 records are all on `POST /v1/invoices`, which polling never calls. |
...
| NFR-4 | Live calls run under a concurrency cap with 429 backoff. | Met in code, and known to be insufficient on one endpoint. `TestClientCapsConcurrencyAtConfiguredLimit` and `TestClientRetriesOn429WithBackoffUpToCap` cover both. The four constants give 1.75s of retry budget in total, and Q5 records an invoice burst quota that takes 45 to 60 seconds to clear, so the seeder waits it out itself rather than the client's backoff reaching it. The constants stay a starting point on every other endpoint. |
...
| Q5 | What rate limit does test mode enforce? | Phase 1 measures it: the first 429 under backoff, recorded with the request rate that produced it. | **Open.** No 429 at 1.4 requests per second over 40 sequential calls on 2026-08-31. The probe is `TestLiveRateLimitObservation`, behind `RZP_RATE_LIMIT_PROBE`. A real ramp is phase 2. **Addendum 2026-09-05, after the pivot: it is a burst quota, not a rate.** `POST /v1/invoices` answered 429 on three independent live seeding attempts that day, and every one of them failed on the sixth invoice creation. Pacing did not move it: the sixth failed at 2.1 writes per second unpaced, at 1.33 with `seed.DefaultPace`, and at 0.80 slower still. An immediate retry failed as well and a retry about 45 seconds later succeeded on its first attempt, which puts the recovery window at 45 to 60 seconds and the quota at about five invoice creations. The first reading of this, written against the first two attempts, called it a rate between 1.4 and 1.9 writes per second; the third attempt at 0.80 falsified that and the sentence is replaced rather than softened. The four backoff constants cannot reach it: `DefaultMaxAttempts` is 4 with 250ms, 500ms and 1s of waiting, which is 1.75s of retry budget against a quota that clears in tens of seconds. The fix is therefore a wait rather than a pace, `seed.DefaultBurstWait` on the calling side, and the runs behind this row are rehearsals that are not committed, so no number here is on a results table. |
